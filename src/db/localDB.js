// localDB.js — complete localStorage database with all features
import { calculateFine } from "../utils/fineCalculator";

// Clear sample demo data on initialization for fresh user start
if (typeof localStorage !== "undefined" && localStorage.getItem("lms_demo_cleared") !== "true") {
  ["lms_books", "lms_members", "lms_transactions", "lms_activity"].forEach((k) => localStorage.removeItem(k));
  localStorage.setItem("lms_demo_cleared", "true");
}
...
// ── SETTINGS ─────────────────────────────────────────────────────────────────
const DEFAULT_SETTINGS = { libraryName: "School Library", fineRatePerDay: 2, defaultLoanDays: 14, maxRenewals: 2 };

export const getSettings = () => {
  try { return { ...DEFAULT_SETTINGS, ...JSON.parse(localStorage.getItem("lms_settings") || "{}") }; }
  catch { return DEFAULT_SETTINGS; }
};
export const saveSettings = (s) => localStorage.setItem("lms_settings", JSON.stringify({ ...getSettings(), ...s }));
...
export const loginUser = (email, password) => {
  if (email === "admin@library.com" && password === "library123") {
...
export const addBook = (data) => {
  const book = { ...data, id: uid(), totalCopies: Number(data.totalCopies), availableCopies: Number(data.totalCopies), createdAt: new Date().toISOString() };
...
export const updateBook = (id, data) => {
  const updated = load("lms_books").map(b => b.id === id ? { ...b, ...data, totalCopies: Number(data.totalCopies ?? b.totalCopies) } : b);
...
export const adjustCopies = (bookId, delta) =>
  save("lms_books", load("lms_books").map(b =>
    b.id === bookId ? { ...b, availableCopies: Math.max(0, (b.availableCopies || 0) + delta) } : b
  ));
...
export const adjustIssued = (memberId, delta) =>
  save("lms_members", load("lms_members").map(m =>
    m.id === memberId ? { ...m, issuedBooks: Math.max(0, (m.issuedBooks || 0) + delta) } : m
  ));
...
export const issueBook = ({ bookId, memberId, bookTitle, memberName, dueDate }) => {
  const txn = { id: uid(), bookId, memberId, bookTitle, memberName, issuedAt: new Date().toISOString(), dueDate, returnedAt: null, fine: 0, finePaid: false, renewals: 0, status: "issued" };
...
export const returnBookTxn = (txnId, { bookId, memberId, dueDate, bookTitle, memberName }) => {
  ...
  save("lms_transactions", load("lms_transactions").map(t =>
    t.id === txnId ? { ...t, returnedAt: new Date().toISOString(), fine, finePaid: fine === 0, status: "returned" } : t
  ));
  adjustCopies(bookId, 1);
  adjustIssued(memberId, -1);
  logActivity("RETURN", `"${bookTitle}" returned by ${memberName}${fine > 0 ? ` — Fine: ₹${fine}` : " — No fine"}`, { bookId, memberId, fine });
  return fine;
};
...
export const renewBookTxn = (txnId, newDueDate) => {
  ...
  if ((txn.renewals || 0) >= maxRenewals) throw new Error(`Maximum renewals (${maxRenewals}) reached`);
  const updated = txns.map(t => t.id === txnId ? { ...t, dueDate: newDueDate, renewals: (t.renewals || 0) + 1 } : t);
  save("lms_transactions", updated);
  logActivity("RENEW", `"${txn.bookTitle}" renewed for ${txn.memberName} — New due: ${new Date(newDueDate).toLocaleDateString()}`, { txnId });
...
export const markFinePaidTxn = (txnId) => {
  ...
  logActivity("FINE_PAID", `Fine of ₹${txn?.fine} marked as paid for "${txn?.bookTitle}" — ${txn?.memberName}`, { txnId, fine: txn?.fine });
};
...
export const importBackup = (data) => {
  ...
  logActivity("RESTORE", `Data restored from backup (exported: ${new Date(data.exportedAt).toLocaleDateString()})`);
};
